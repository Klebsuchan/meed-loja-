...
const newImports = `import { motion, useScroll, useTransform, AnimatePresence } from 'motion/react';
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const prods = querySnapshot.docs.map(doc => {
        ...
        return {
          id: doc.id,
          ...data,
          title: data.name || data.title || "Produto Destacado",
  ...
  return (
    <section ref={ref} className="relative min-h-[100dvh] w-full overflow-hidden flex items-center justify-center bg-[#050505] pt-32 pb-24 md:pt-32 md:pb-12">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-center md:justify-between px-6 md:px-12 w-full max-w-7xl h-full gap-8 md:gap-0 mt-8 md:mt-0">
        ...
        {/* Left Content */}
        <motion.div style={{ y: textY }} className="flex flex-col items-center text-center md:items-start md:text-left w-full md:w-1/2 pt-16 md:pt-0">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
        ...
        <motion.div style={{ y: carouselY }} className="w-full md:w-1/2 flex flex-col justify-center md:justify-end items-center md:items-end relative mt-12 md:mt-0">
          <AnimatePresence mode="wait">
            <motion.div
...
fs.writeFileSync('src/components/ParallaxHero.tsx', newImports);
